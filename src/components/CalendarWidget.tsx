...
export default function NotesManager() {
  ...
  async function handleSave() {
    if (!activeNote || !activeNote.title) return alert("Inserisci un titolo per la nota.");
  ...
  async function uploadFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (activeNote.id) {
        await supabase.from('notes').update({ attachments: updatedAttachments }).eq('id', activeNote.id);
      }
    } catch (err: any) {
      ...
      <div className={`w-full md:w-80 border-r border-white/5 flex flex-col ${activeNote ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {loading ? <div className="p-4 text-center"><Loader2 className="animate-spin inline" /></div> : 
            ...
            <button key={note.id} onClick={() => setActiveNote(note)} className={`w-full text-left p-4 border-b border-white/5 transition-all ${activeNote?.id === note.id ? 'bg-[#FF914D]/10' : 'hover:bg-white/[0.02]'}`}>
        ...
        {activeNote ? (
          <>
            <div className="p-4 border-b border-white/5 flex justify-between items-center">
              <button onClick={() => setActiveNote(null)} className="md:hidden text-[#FF914D] flex items-center gap-1 text-[10px] font-black"><ChevronLeft size={16} /> NOTE</button>
              <div className="flex items-center gap-4 ml-auto">
                <label className="cursor-pointer text-zinc-500 hover:text-white transition-colors">
                  {isUploading ? <Loader2 size={18} className="animate-spin" /> : <Paperclip size={18} />}
                  <input type="file" className="hidden" onChange={uploadFile} />
                </label>
                <button onClick={handleSave} disabled={isSaving} className="text-[#FF914D] hover:scale-110 transition-transform">
                  {isSaving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
                </button>
                <button onClick={() => handleDelete(activeNote.id)} className="text-zinc-800 hover:text-red-500"><Trash2 size={18} /></button>
              </div>
            </div>
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-4 text-white">
              <input type="text" placeholder="TITOLO" value={activeNote.title} onChange={e => setActiveNote({...activeNote, title: e.target.value})} className="w-full bg-transparent text-2xl font-black uppercase italic outline-none placeholder:text-zinc-800" />
              <textarea placeholder="Inizia a scrivere..." value={activeNote.content} onChange={e => setActiveNote({...activeNote, content: e.target.value})} className="w-full bg-transparent flex-1 resize-none outline-none font-mono text-sm leading-relaxed placeholder:text-zinc-900 min-h-[300px]" />
                ...
                <div className="grid grid-cols-2 gap-2 pt-4 border-t border-white/5">
                  {activeNote.attachments.map((url: string, i: number) => (
                    <div key={i} className="relative group rounded-xl overflow-hidden border border-white/10">
                      ...
                      <a href={url} target="_blank" className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity"><FileText size={20} /></a>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-800 uppercase font-black italic">
            <FileText size={48} className="mb-4 opacity-20" /> Seleziona una nota
          </div>
        )}
      </div>
    </div>

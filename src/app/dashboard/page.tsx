...
import { 
  ...
  Layers, Loader2, ChevronRight, Sparkles 
} from 'lucide-react';
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="glass-panel p-6 border-white/5 bg-zinc-900/20 rounded-3xl">
              ...
              <form onSubmit={handleCreatePost} className="space-y-4">
                <input type="text" placeholder="UNIT_TITLE" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-black/40 border border-white/5 p-4 rounded-xl font-mono text-[10px] uppercase text-white outline-none" />
                <div className="relative group">
                  <textarea placeholder="DESCRIPTION_DRAFT..." value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-black/40 border border-white/5 p-4 pr-12 rounded-xl font-mono text-[10px] min-h-[100px] text-white outline-none resize-none" />
                  <button type="button" onClick={handleAiEnhance} disabled={isAiProcessing || !description} className="absolute right-3 bottom-3 p-2 bg-zinc-800 hover:bg-[#FF914D] text-white rounded-lg transition-all">
                ...
                <button type="submit" disabled={uploading} className="w-full py-4 bg-white text-black text-[10px] font-black uppercase italic hover:bg-[#FF914D] transition-all">
                  {uploading ? <Loader2 className="animate-spin mx-auto" size={16} /> : 'PUSH_TO_FACTORY'}
                </button>
              </form>
            </div>

            <button onClick={() => router.push('/dashboard/outputs')} className="w-full group p-6 bg-zinc-900/20 border border-white/5 rounded-3xl flex items-center justify-between hover:bg-white/[0.02]">
